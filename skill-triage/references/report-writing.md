...
7. `## 需要你决定`: list Agent-reviewed trade-offs where the answer depends on the user's habits. This section is not an execution list.
8. `## 建议下一步`: give concrete next actions in priority order.
9. `## 没有修改的内容`: state that SkillTriage only prepared reviewable files.
...
Do not include internal report-review commentary in `report.md`. Avoid self-review hedges, implementation-note phrases, and future-iteration musings. The report is for the user; keep those notes out of `report.md`.
...
当 Agent 评估发现真实取舍，但答案取决于用户习惯时，在可选整理执行之前加入这一节。这些项目不是高置信清理项，不要混进清理清单。

Write it in practical Chinese:
...
边界要写清楚: 偏好只影响提示和排序，不会自动清理 skill，也不能替代本次 Agent 评估。不要把弱证据写成归档建议。即使用户选择 `archive_candidate`，它也只是一个决策选项；只有进入单独的可选整理执行流程，并且用户明确批准具体动作后，才可能变成可执行动作。
