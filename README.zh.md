...
| [shinkaskill](./shinkaskill) | Codex / Claude Code | 检查 Agent Skill 的结构、触发描述、引用文件和真实 eval 结果，生成中文报告 |
...
- `shinkaskill` 需要 Node.js 20+。真实 eval 需要当前环境里已登录的 Codex CLI 或 Claude Code CLI；没有可用 adapter 时，它会如实写明不可用原因，不会把 fallback 写成真实运行结果。
...
Agent skill 一多，问题往往不会只出在语法上。触发描述太宽、引用文件缺失、执行边界不清、报告只给结论不给证据，都会让 agent 在真正使用时变得不稳定。ShinkaSkill 用来把这些问题拆开检查，给出一份可以审阅、可以复测的中文报告。

#### 它做什么

ShinkaSkill 会检查指定 skill 的 `SKILL.md`、metadata、引用文件、脚本、用户授权说明和可维护性，并按 rubric 逐项打分。用户明确同意后，它可以通过 Codex 或 Claude Code adapter 启动 subagent 做真实 eval，再把 grader 和 comparator 结果写进报告。

默认模式只读，不会改动原始 skill。`propose` 和 `apply` 是单独入口，后续要写回时也会重新请求用户确认。
