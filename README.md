...
这个仓库收集了一组 `SKILL.md` 风格的 AI Agent skill 包，适合复制到 Claude Code、Codex、Hermes 等支持 Markdown 指令的 agent runtime 中使用。

每个 skill 都围绕一个具体工作流组织：核心指令放在 `SKILL.md`，必要的参考资料、模板和脚本放在同一个文件夹里。这样 agent 在处理类似任务时，可以直接复用一套稳定流程。仓库里的 skill 会尽量保持可迁移；如果某个 skill 依赖特定 runtime 或外部工具，会在对应说明里标出来。
...
| [shinkaskill](./shinkaskill) | Codex / Claude Code | 检查 Agent Skill 的结构、触发描述、引用文件和真实 eval 结果，按 rubric 生成中文报告和改进建议 |
...
- `shinkaskill` 需要 Node.js 20+。只做静态检查时不依赖外部 agent；真实 eval 需要当前环境里已登录的 Codex CLI 或 Claude Code CLI。adapter 不可用时，它会写明原因，不会把 fallback 写成真实运行结果。
...
Skill 写完以后，最容易漏掉的是运行时问题：触发条件是否太宽，说明是否让 agent 犹豫，引用文件是否缺失，脚本和权限边界是否说清，真实任务里是否真的能完成。人工翻 `SKILL.md` 很难稳定发现这些问题，也很难把判断量化。

ShinkaSkill 用来给 Agent Skill 做发布前检查。它先把结构、说明、引用和运行边界拆开看，再把分数、证据和改进建议写成中文报告，方便继续修改或交给别人审阅。

#### 它做什么

ShinkaSkill 接收一个或多个 skill 路径，先做只读检查：读取 `SKILL.md`、frontmatter、引用文件和脚本，检查结构完整性、触发描述、渐进加载、授权说明、runtime 绑定和可维护性。报告默认用中文生成，并按 rubric 给每个维度打分。

需要更接近真实使用时，可以让用户明确授权真实 eval。ShinkaSkill 会把被测 skill 复制到 run sandbox，通过 Codex 或 Claude Code adapter 启动 subagent 跑 prompts，再让 grader 逐项打分、comparator 做 A/B 判断。报告会区分静态检查、真实 eval 和 adapter 不可用的情况，避免把没跑过的结果写成已验证。
