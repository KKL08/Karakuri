...
| [coding-agent-fit](./coding-agent-fit) | 通用 coding agent | 评测云服务、API 平台或开发者工具对 Coding Agent 接入的支持程度 |
...
当 Claude Code、Cursor、Codex、Trae 成为常用开发工具，开发者接入云服务的方式也在变化：很多时候，人会把文档交给 Agent，让它搜索服务入口、读取资料并生成集成代码。

这个 skill 用来判断服务方给出的入口、文档、API、SDK、CLI、MCP 和 Skill，能不能支撑 Agent 把接入真正跑起来。

#### 它做什么

输入一个云服务或开发工具的文档 URL，输出一份接入评测报告，说明 Coding Agent 在接入时大概有多顺、最可能卡在哪里、服务方最该先补什么。

适用于：

- **DevRel / 文档团队**：检查自家服务对 Coding Agent 接入的支持情况
- **Agent 开发者**：快速判断一个平台是否适合交给 Agent 集成

评分覆盖服务入口、接入文档、Agent 辅助工具、接入阻碍、维护与反馈 5 个维度。
