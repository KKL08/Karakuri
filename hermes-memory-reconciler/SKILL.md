...
当前仓库里的 `memory-reconciler` CLI 已实现只读扫描、摘要、下一问题、用户裁决、dry-run plan 和 `preview <plan_id>`。

`stage <plan_id>`、`preview <run_id>`、`apply <run_id>`、`rollback <run_id>` 属于后续 staged run 生命周期。如果 CLI 返回 `not_implemented`，停在 dry-run plan，不要手动伪造 `run_id`、run 目录或 manifest。
...
- 当前 CLI 如果对 staged run 命令返回 `not_implemented`，停在阶段 2 的 dry-run plan。
- 后续 `stage <plan_id>` 会写入独立工作目录并生成 `run_id`，但仍不修改 Hermes 源记忆。
...
- 如果要生成候选文件，优先 `stage <plan_id>`；若返回 `not_implemented`，停在 dry-run plan。
- 如果 `stage <plan_id>` 已可用，必须确认它返回 `run_id`，且 run 目录里有 `original/`、`proposed/`、`diffs/`、`manifest.json`。
...
CLI 写到 `~/.memory-reconciler/` 的 artifact 可能保留 raw memory、`old_text` 和锚点信息，用于后续匹配。默认只把 stdout、`report` 和 `preview` 的遮盖后摘要展示给用户；不要把 artifact 原文贴进对话、上传或提交到仓库。
...
危险指令型记忆不要和普通记忆合并。先标记、必要时遮盖 credential-like 内容，再给出 remove 的 dry-run 建议；只有后续 staged lifecycle 明确定义 quarantine 语义后，才使用 quarantine action。
